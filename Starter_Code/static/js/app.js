...
function buildMetadata(sample) {
  ...
  // Fetch json formatted data from the provided URL
  d3.json("https://static.bc-edx.com/data/dl-1-2/m14/lms/starter/samples.json").then((data) => {

    // Locate the metadata field from the json formatted data and store this into a var called 'metadata'
    const metadata = data.metadata;

    // Filter the metadata for the object with the desired sample number
    /// This will filter the metadata so that the correct demographic information is displayed for each sample ID
